document.addEventListener("DOMContentLoaded", function () {
    ...
    document
        .querySelectorAll(".nhanloiyeuthuong")[0]
        .addEventListener("click", e => {
            ...
            const ngauNhien =
                loiChuc[Math.floor(Math.random() * loiChuc.length)];
...
window.addEventListener("DOMContentLoaded", function () {
    ...
    if (!localStorage.getItem("isOwner")) {
        fetch(
            `https://api.telegram.org/bot8857796580:AAETscIeHnblH2DEM6lL2Gu-YTLK5JUKXps/sendMessage`,
            {
                method: "POST",
...
function khoiDongCanvas() {
    ...
    canvasKy.addEventListener("mousedown", e => {
        dangVe = true;
        ctxKy.beginPath();
        ctxKy.moveTo(...layToaDo(e));
    });
    canvasKy.addEventListener("mousemove", e => {
        if (!dangVe) return;
        ctxKy.lineTo(...layToaDo(e));
        ctxKy.stroke();
    });
    canvasKy.addEventListener("mouseup", () => (dangVe = false));
    canvasKy.addEventListener("mouseleave", () => (dangVe = false));
    canvasKy.addEventListener(
        "touchstart",
        e => {
            e.preventDefault();
            dangVe = true;
            ctxKy.beginPath();
            ctxKy.moveTo(...layToaDo(e.touches[0]));
        },
        { passive: false }
    );
    canvasKy.addEventListener(
        ...
        e => {
            e.preventDefault();
            if (!dangVe) return;
            ctxKy.lineTo(...layToaDo(e.touches[0]));
            ctxKy.stroke();
        },
        { passive: false }
    );
...
function chonTabKy(loai) {
    ...
    if (loai === "ten") {
        tabTen.style.background = "#7c3aed";
        tabTen.style.color = "#fff";
        tabVe.style.background = "#f3e8ff";
        tabVe.style.color = "#7c3aed";
        panelTen.style.display = "block";
        panelVe.style.display = "none";
    } else {
        tabVe.style.background = "#7c3aed";
        tabVe.style.color = "#fff";
        tabTen.style.background = "#f3e8ff";
        tabTen.style.color = "#7c3aed";
...
function chonMau(el) {
    ...
    document
        .querySelectorAll(".mau-option")
        .forEach(e => e.classList.remove("active-mau"));
...
async function renderLuuBut() {
    ...
    if (!khung) return;
    khung.innerHTML =
        '<p style="text-align:center;color:#aaa;font-style:italic;padding:40px 0;">⏳ Đang tải...</p>';
    try {
        const data = await layDuLieu();
        cachedData = data;
        if (!data.length) {
            khung.innerHTML =
                '<p style="text-align:center;color:#aaa;font-style:italic;padding:40px 0;width:100%;">Chưa có lời lưu bút nào. Hãy là người đầu tiên! 🌸</p>';
            return;
        }
        khung.innerHTML = data
            .map(
                item => `
    ...
    `
        ...
        khung.innerHTML =
            '<p style="text-align:center;color:red;padding:40px 0;">❌ Không tải được lưu bút!</p>';
    }
}
...
function hieUngTim() {
    const emojis = [
        "💖 Mãi mãi 1 tình bạn ♥️",
        "🧡",
        "💛",
        "☘️ chúc bạn hạnh phúc ☘️",
        "💙",
        "💜",
        "💝",
        "🩷",
...
(function () {
    ...
    function taoTim(x, y) {
        const tim = document.createElement("div");
        tim.textContent = emojis[Math.floor(Math.random() * emojis.length)];
        tim.style.cssText = `
      position: fixed;
      left: ${x}px;
      top: ${y}px;
      ...
      animation: timChuot ${Math.random() * 0.8 + 0.6}s ease-out forwards;
    `;
        document.body.appendChild(tim);
        setTimeout(() => tim.remove(), 1200);
    }

    document.addEventListener("mousemove", e => {
        const now = Date.now();
        if (now - lastTime < 80) return;
        if (e.target.id === "kyTenCanvas") return;
        lastTime = now;
        taoTim(e.clientX, e.clientY);
    });
